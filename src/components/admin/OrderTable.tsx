...
import { MoreHorizontal, MessageSquare, Loader2, Trash2 } from 'lucide-react';
...
export default function OrderTable({ orders }: OrderTableProps) {
  ...
  const confirmDelete = async () => {
    ...
    setUpdatingId(`delete-${orderToDelete.id}`);
    startTransition(async () => {
        try {
            const orderRef = doc(db, 'orders', orderToDelete.id);
            await deleteDoc(orderRef);
            toast({ title: 'Order Deleted', description: `Order ${orderToDelete.id} has been permanently deleted.`});
            setOrderToDelete(null);
        } catch (error: any) {
            toast({ title: 'Delete Failed', description: "You do not have permission to delete this order.", variant: 'destructive'});
        } finally {
            setUpdatingId(null);
        }
    });
  ...
  const isUpdating = (type: 'status' | 'payment' | 'delete', orderId: string) => {
      ...
      <AlertDialog open={!!orderToDelete} onOpenChange={() => setOrderToDelete(null)}>
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    This action cannot be undone. This will permanently delete the order for <span className="font-bold">{orderToDelete?.fullName}</span>.
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={confirmDelete} className="bg-destructive hover:bg-destructive/90" disabled={isPending && updatingId?.includes('delete')}>
                  {isPending && updatingId?.includes('delete') ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                  Delete
                </AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
